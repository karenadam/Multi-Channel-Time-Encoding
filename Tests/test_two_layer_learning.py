...
class TestLearningWorksOneExample:
    ...
    def test_4_signals_can_find_spike_times(self):
        ...
        f_s_coefficients[0,:] = fri_signal_1.get_fourier_series(np.arange(-n_f_s+1, n_f_s, 1).T)
        f_s_coefficients[1,:] = fri_signal_2.get_fourier_series(np.arange(-n_f_s+1, n_f_s, 1).T)
        f_s_coefficients[2,:] = fri_signal_3.get_fourier_series(np.arange(-n_f_s+1, n_f_s, 1).T)
        f_s_coefficients[3,:] = fri_signal_4.get_fourier_series(np.arange(-n_f_s+1, n_f_s, 1).T)
