...
class TestTimeEncoderMultiSignalMultiChannel:
    ...
    def test_can_reconstruct_standard_encoding_with_2_by_2_mixing_one_shot(self):
        ...
        tem_mult = timeEncoder(kappa, delta, b, A, integrator_init=int_shift)
        spikes_mult = tem_mult.encode_precise(signals, t[-1])
    ...
    def test_can_reconstruct_precise_encoding_with_3_by_2_mixing_recursive(self):
        ...
        y = np.atleast_2d(y)
        A = [[0.9, 0.1], [0.2, 0.8], [1, 1]]
        y_param = bandlimitedSignals(omega, sinc_locs=[], sinc_amps=[])
        ...
        tem_mult = timeEncoder(kappa, delta, b, A, integrator_init=int_shift)
        spikes_mult = tem_mult.encode_precise(y_param, end_time)
    ...
    def test_can_reconstruct_precise_encoding_with_3_by_2_mixing_one_shot_no_fixed_sinc_locs(
        ...
        rec_mult = tem_mult.decode_mixed(
            spikes_mult, t, Omega=omega, sinc_locs=original1.get_sinc_locs(), Delta_t=0
        )
