...
class TestFriSignal:
    def test_proper_creation(self):
        fri_signal = src.FRISignal.FRISignal(np.array([0,1,1.5]),np.array([1,1,1]), 2.0)
    ...
    def test_proper_FS_coeffs_1(self):
        ...
        m = np.array([[1, 2, 5]]).T
        assert np.allclose(fri_signal.get_fourier_series(m), 1/2 * np.exp(-1j*2*np.pi*m/2.0).T)
    ...
    def test_proper_FS_coeffs_2(self):
        ...
        assert np.allclose(fri_signal.get_fourier_series(m),1/2* (np.exp(-1j*2*np.pi*m/2.0)-np.exp(-1j*2*1.5*np.pi*m/2.0)).T)
    ...
    def test_make_signal_annihilatable(self):
        ...
        result_1 = np.convolve(a_filter.get_filter_coefficients(), f_s_coefficients[0,:])
        result_2 = np.convolve(a_filter.get_filter_coefficients(), f_s_coefficients[1,:])
        result_2_updated = np.convolve(a_filter.get_filter_coefficients(), annihilatable_f_s)
        assert np.allclose(result_1[M:-M], np.zeros_like(result_1[M:-M]))
        assert not np.allclose(result_2[M:-M], np.zeros_like(result_2[M:-M]))
        assert np.allclose(result_2_updated[M:-M], np.zeros_like(result_2_updated[M:-M]))
    ...
    def test_random_sensing_of_f_s_coefficients(self):
        ...
        random_sensing_matrix = np.random.random(size = (len(f_s_coefficients_flattened), len(f_s_coefficients_flattened)))
    ...
    def test_can_find_annihilator_4_signals(self):
        ...
        f_s_coefficients[0,:] = fri_signal_1.get_fourier_series(np.arange(-M, M+1,1).T)
        f_s_coefficients[1,:] = fri_signal_2.get_fourier_series(np.arange(-M, M+1,1).T)
        f_s_coefficients[2,:] = fri_signal_3.get_fourier_series(np.arange(-M, M+1,1).T)
        f_s_coefficients[3,:] = fri_signal_4.get_fourier_series(np.arange(-M, M+1,1).T)
        ...
        target_coefficients = np.product(1 - np.exp(-1j*2*np.pi*t_k_1/2.0)*np.exp(-1j*2*np.pi*K_s/filter_length), 1)
        print("TARGET: ", target_coefficients)
        print("RESULT: ", np.fft.fft(a_filter.get_filter_coefficients()))

        assert np.allclose(target_coefficients,np.fft.fft(a_filter.get_filter_coefficients()))
    ...
    def get_circular_matrix(self, hidden_f_s_coefficients, num_diracs: int):
        ...
        for i in range(num_diracs+2):
            # has been checked
            circular_matrix[i,:] = hidden_f_s_coefficients[i:num_diracs+i+2][::-1]
        return circular_matrix


    def recover_from_meas(self, random_sensing_matrices: list, measurements: list, num_diracs: int, num_signals: int):
        ...
        circularity_constraints = np.zeros((num_signals*n_circularity_constraint_per_signal, np.product(recovered_matrix_shape)))
        # This has been verified already
        for n_s in range(num_signals):
            for i in range(num_diracs+1):
                for j in range(1,num_diracs+2):
                    circularity_constraints[n_s*n_circularity_constraint_per_signal + i*(num_diracs+2)+j-1, n_s*(num_diracs+2)**2 + (i+1)*(num_diracs+2) +j] = 1
                    circularity_constraints[n_s*n_circularity_constraint_per_signal+ i*(num_diracs+2)+j-1,n_s*(num_diracs+2)**2 + i*(num_diracs+2)+j-1] = -1
        concatenated_measurements = np.zeros((circularity_constraints.shape[0],))

        projection_constraints = np.zeros((0,np.product(recovered_matrix_shape)), dtype = 'complex')
        # print(projection_constraints.shape)
        for n_s_m in range(num_signals):
            random_sensing_matrix = random_sensing_matrices[n_s_m]
            projection_constraints = np.concatenate((np.zeros((random_sensing_matrix.shape[0], np.product(recovered_matrix_shape)), dtype = 'complex'), projection_constraints))
            for i in range(random_sensing_matrix.shape[0]):
                # has been checked
                projection_constraints[i,n_s_m*(num_diracs+2)**2 + 0: n_s_m*(num_diracs+2)**2 + num_diracs+2] = random_sensing_matrix[i,0:num_diracs+2][::-1]
                projection_constraints[i,(n_s_m+1)*(num_diracs+2)**2-(num_diracs+2):(n_s_m+1)*(num_diracs+2)**2-1] = random_sensing_matrix[i, num_diracs+2::][::-1]

            concatenated_measurements = np.concatenate((measurements[n_s_m], concatenated_measurements))
        concatenated_constraints = np.concatenate((projection_constraints, circularity_constraints))
        circ_matrix_recovered = src.Helpers.singular_value_projection_w_matrix(recovered_matrix_shape, concatenated_constraints, concatenated_measurements, num_diracs+1, tol=1e-3, lr=0.5)
