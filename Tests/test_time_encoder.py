...
class TestTimeEncoderSingleSignalMultiChannel:
    def test_can_reconstruct_standard_encoding_ex1(self):
        ...
        tem_mult = timeEncoder(
            kappa, delta, b, n_channels=4, integrator_init=[int_shift]
        )
    ...
    def test_can_reconstruct_standard_encoding_with_recursive_mixing_alg(self):
        ...
        tem_mult = timeEncoder(kappa, delta, b, n_channels=2, integrator_init=int_shift)
        spikes_mult = tem_mult.encode_multi_signal(y, A, delta_t)
        rec_mult = tem_mult.decode_multi_signal_multi_channel_recursive(
            spikes_mult, A, t, omega, delta_t, num_iterations=20
        )
...
class TestTimeEncoderMultiSignalMultiChannel:
    def test_can_compute_q_matrix(self):
        ...
        tem_mult = timeEncoder(kappa, delta, b, n_channels=2, integrator_init=int_shift)
        spikes_mult = tem_mult.encode_multi_signal(y, A, delta_t)
        q1, G = tem_mult.get_closed_form_matrices_multi_signal_multi_channel(
            spikes_mult, np.array(A), omega
        )
    ...
    def test_can_reconstruct_standard_encoding_with_3_by_2_mixing_recursive(self):
        ...
        tem_mult = timeEncoder(kappa, delta, b, n_channels=3, integrator_init=int_shift)
        spikes_mult = tem_mult.encode_multi_signal(y, A, delta_t)
        rec_mult = tem_mult.decode_multi_signal_multi_channel_recursive(
            spikes_mult, A, t, omega, delta_t, num_iterations=20
        )
    ...
    def test_params_TEM_correct(self):
        TEM = timeEncoder(kappa=1, delta=1, b=1, n_channels=2)
