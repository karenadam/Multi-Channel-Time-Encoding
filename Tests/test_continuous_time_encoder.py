...
class TestContinuousTimeEncoder:
    ...
    def test_consistent_constraints_sos(self):
        ...
        q = decoder.SSignalMChannelDecoder(tem_params, Omega = omega).get_measurement_vector(spikes_single)
        q_gt = original.get_precise_integral(spikes_single[0][:-1], spikes_single[0][1:])
    ...
    def test_consistent_constraints_per(self):
        ...
        q = decoder.SSignalMChannelDecoder(tem_params, periodic = True, period = period, n_components=n_components).get_measurement_vector(spikes_single)
        q_gt = original.get_precise_integral(spikes_single[0][:-1], spikes_single[0][1:])
