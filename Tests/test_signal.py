...
class TestPiecewiseConstantSignals():
    ...
    def test_sampling(self):
        ...
        samples_1_loop_approach = signal_1.low_pass_filter(np.pi).sample(np.array(sample_locs))

        assert np.linalg.norm(samples_1_matrix_approach - samples_1_loop_approach)<1e-6
