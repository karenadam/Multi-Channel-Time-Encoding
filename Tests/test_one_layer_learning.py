...
class TestLearningWorksOneSignal:
    def test_one_signal_2_by_2(self):
        ...
        np.random.seed(10)
        original1 = Signal.periodicBandlimitedSignal(10, 10, np.random.random(size = (10,)) )
        np.random.seed(11)
        original2 = Signal.periodicBandlimitedSignal(10, 10, np.random.random(size = (10,)) )
...
class TestLearningWorksMultiSignal:
    def test_multi_signal_2_by_2(self):
        ...
        signals_1.add(Signal.periodicBandlimitedSignal(10, 10, np.random.random(size = (10,)) ))
        signals_1.add(Signal.periodicBandlimitedSignal(10, 10, np.random.random(size = (10,)) ))
        A = [[0.9, 0.1], [0.2, 0.8]]


        signals_2 = Signal.periodicBandlimitedSignals(10)
        signals_2.add(Signal.periodicBandlimitedSignal(10, 10, np.random.random(size = (10,)) ))
        signals_2.add(Signal.periodicBandlimitedSignal(10, 10, np.random.random(size = (10,)) ))
        ...
        single_layer = Layer(2,2)
        single_layer.learn_weight_matrix_from_multi_signals([signals_1, signals_2], [spikes_mult_1, spikes_mult_2])
